...
windows-exe-release:
	fastforge package \
	  ...
	  --build-target=$(TARGET) \
	  --build-dart-define=sentry_dsn=$(SENTRY_DSN)
	@pwsh -NoLogo -NoProfile -NonInteractive -Command '$$ErrorActionPreference = "Stop"; $$kitsBin = Join-Path $${env:ProgramFiles(x86)} "Windows Kits\10\bin"; if (-not (Test-Path -LiteralPath $$kitsBin -PathType Container)) { throw "Windows Kits 10 bin directory was not found: $$kitsBin" }; $$signtoolCandidates = @(Get-ChildItem -LiteralPath $$kitsBin -Directory | ForEach-Object { $$sdkVersion = $$null; if ([Version]::TryParse($$_.Name, [ref]$$sdkVersion)) { $$candidatePath = Join-Path $$_.FullName "x64\signtool.exe"; if (Test-Path -LiteralPath $$candidatePath -PathType Leaf) { [pscustomobject]@{ Version = $$sdkVersion; Path = $$candidatePath } } } }); if ($$signtoolCandidates.Count -eq 0) { throw "No versioned x64 signtool.exe was found under $$kitsBin" }; $$selectedSigntool = $$signtoolCandidates | Sort-Object Version -Descending | Select-Object -First 1; $$pfxPath = Join-Path $$PWD "windows\sign.pfx"; $$configPath = Join-Path $$PWD "windows\packaging\msix\make_config.yaml"; if (-not (Test-Path -LiteralPath $$pfxPath -PathType Leaf)) { throw "Windows signing certificate was not found." }; if (-not (Test-Path -LiteralPath $$configPath -PathType Leaf)) { throw "Windows packaging config was not found." }; $$passwordLines = @(Get-Content -LiteralPath $$configPath | Where-Object { $$_ -match "^\s*certificate_password\s*:" }); if ($$passwordLines.Count -ne 1) { throw "Expected exactly one active certificate_password entry in Windows packaging config." }; $$certificatePassword = ($$passwordLines[0] -replace "^\s*certificate_password\s*:\s*", "").Trim(); if ([string]::IsNullOrWhiteSpace($$certificatePassword)) { throw "certificate_password is empty in Windows packaging config." }; $$pfxCertificates = [System.Security.Cryptography.X509Certificates.X509Certificate2Collection]::new(); $$pfxCertificates.Import($$pfxPath, $$certificatePassword, [System.Security.Cryptography.X509Certificates.X509KeyStorageFlags]::EphemeralKeySet); $$privateKeyCertificates = @($$pfxCertificates | Where-Object { $$_.HasPrivateKey }); if ($$privateKeyCertificates.Count -ne 1) { throw "Expected exactly one private-key certificate in the signing PFX." }; $$expectedThumbprint = $$privateKeyCertificates[0].Thumbprint; $$setupFiles = @(Get-ChildItem -LiteralPath (Join-Path $$PWD "dist") -Recurse -File -Filter "*-windows-setup.exe"); if ($$setupFiles.Count -ne 1) { throw "Expected exactly one *-windows-setup.exe under dist; found $$($$setupFiles.Count)." }; $$setupPath = $$setupFiles[0].FullName; & ($$selectedSigntool.Path) sign /fd SHA256 /tr "http://timestamp.digicert.com" /td SHA256 /f $$pfxPath /p $$certificatePassword $$setupPath; $$signExitCode = $$LASTEXITCODE; $$certificatePassword = $$null; if ($$signExitCode -ne 0) { throw "signtool sign failed with exit code $$signExitCode." }; $$signature = Get-AuthenticodeSignature -LiteralPath $$setupPath; $$rejectedStatuses = @("NotSigned", "HashMismatch", "NotSupportedFileFormat", "Incompatible"); if ($$null -eq $$signature.SignerCertificate) { throw "The Windows setup EXE has no signer certificate." }; if ($$rejectedStatuses -contains [string]$$signature.Status) { throw "The Windows setup EXE signature status is $$($$signature.Status)." }; if (-not [string]::Equals($$expectedThumbprint, $$signature.SignerCertificate.Thumbprint, [StringComparison]::OrdinalIgnoreCase)) { throw "The Windows setup EXE signer does not match the signing PFX certificate." }; if ($$null -eq $$signature.TimeStamperCertificate) { throw "The Windows setup EXE has no RFC3161 timestamp certificate." }; $$pfxCertificates | ForEach-Object { $$_.Dispose() }; Write-Host ("Embedded signature, signer, and RFC3161 timestamp matched for {0} with Windows SDK {1}. Authenticode status: {2}; this check does not establish publisher trust." -f $$setupPath, $$selectedSigntool.Version, $$signature.Status)'
